...
private:
	...
	void Merge_PO_Parallel(std::vector<Polygon*>& po_polygons, int thread_count) {
		...
		std::string name = input.layer_id_to_name[input.gate_rule.first];
		QuadTree* quad_tree = new QuadTree(input.layout, MAX_DEPTH, MAX_DATA_NUM, "PO:" + name);
		quad_tree->CreatIndexParallel(po_polygons, thread_count);
		// 相交检测求边
	...
	{	
    	...
    	// 用于线程安全地收集结果的容器
    	std::vector<std::vector<Polygon*>> thread_results(thread_count);
    	robin_hood::unordered_map<int, std::vector<Edge>> global_po_cut_edges;
		
    	// 并行处理每个AA多边形
    	#pragma omp parallel num_threads(thread_count)
    	{
    	    int tid = omp_get_thread_num();
    	    std::vector<Polygon*>& local_polys = thread_results[tid];
    	    ...
    	    for (int i = 0; i < aa_polygons.size(); i++) {
    	        ...
    	        // 未被任何PO切割
    	        if (it == aa_cut_by_po.end()) {
    	            local_polys.push_back(poly_ptr[aa_id]);
    	            continue;
    	        }
    	        // 若被PO切割
    	        ...
    	        if (cutting_po.size() == 1) {
    	            ...
    	            for (auto& poly : cut_poly_set) {
    	                ...
    	                new_poly->rect = input.GetRectofPolygon(new_poly);
    	                local_polys.push_back(new_poly);
    	            }
    	    ...
    	    {
    	        // 计算当前线程结果在全局中的偏移量
    	        size_t offset = aa_cut_polygons.size();
    	        aa_cut_polygons.insert(aa_cut_polygons.end(), 
    	                              local_polys.begin(), local_polys.end());
    	        ...
    	        for (auto& pair : local_edges) {
    	            ...
    	            for (Edge& edge : pair.second) {
    	                edge.first += offset;
    	                edge.second += offset;
    	                global_po_cut_edges[po_id].push_back(edge);
    	            }
    	        }
    	    }
    	}
    	...
    	// 更新结果
    	po_cut_edges = std::move(global_po_cut_edges);
    	aa_polygons = std::move(aa_cut_polygons);
